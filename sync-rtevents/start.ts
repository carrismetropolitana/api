...
export default async () => {
	...
	const allRtEventsUpdated = new Map();
	...
	for (const pcgiVehicleEvent of allPcgiVehicleEvents) {
		...
		if (allRtEventsUpdated.get(vehicleId) && allRtEventsUpdated.get(vehicleId).timestamp >= vehicleTimestamp) continue;
		...
		allRtEventsUpdated.set(vehicleId, {
	...
	LOGGER.info(`Parsed all Vehicle Events into ${allRtEventsUpdated.size} unique RT Events (${parseTimer.get()})`);

	// 4.
	// Prepare the RT Events data in JSON and Protobuf formats

	const conversionsTimer = new TIMETRACKER();

	const allRtEventsUpdatedArray = Array.from(allRtEventsUpdated.values());

	const allRtEventsUpdatedJson = convertToJson(allRtEventsUpdatedArray);
	await SERVERDB.client.set(`rtevents:json`, JSON.stringify(allRtEventsUpdatedJson));

	const allRtEventsUpdatedProtobuf = convertToProtobuf(allRtEventsUpdatedArray);
	await SERVERDB.client.set(`rtevents:protobuf`, JSON.stringify(allRtEventsUpdatedProtobuf));

	LOGGER.info(`Converted RT Events into JSON and Protobuf formats (${conversionsTimer.get()})`);
