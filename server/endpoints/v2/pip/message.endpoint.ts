	...
	for (const stopId of request.body.stops) {
		...
		if (stopId === '000000') {
			return reply
				...
				.send({ message: `>> TESTE. PIP ID ${request.params.id} | ${DateTime.local({ zone: 'Europe/Lisbon' }).toFormat('HH:mm:ss')}` });
		}
		return reply
			.code(200)
			.header('Content-Type', 'application/json; charset=utf-8')
			.send({ message: `Painel em Testes | PIP ID ${request.params.id} | ${DateTime.local({ zone: 'Europe/Lisbon' }).toFormat('HH:mm:ss')}` });
	}
};

/* * */

FASTIFY.server.get('/v2/pip/:id/message', main);
