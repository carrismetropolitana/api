...
class RTEVENTS {
  ...
  async update() {
    ...
    //
    // 4. Set the current time to the last_update flag to avoid over fetching

    this.last_update = DateTime.now().toUnixInteger();

    //
    // 5. Reset the Map variable

    const updatedRtEvents = new Map();

    //
    // 6. Update vehicles with the latest events

    for (const rtEvent of allRtEvents) {
      //

      //
      // 6.1. Perform basic event validations
      ...
      //
      // 6.3. Check if there is a vehicle already saved and that it has an older timestamp than the current event

      if (this.rt_events[vehicleId] && this.rt_events[vehicleId].timestamp >= vehicleTimestamp) continue;

      //
      // 6.4. Save the current event
    ...
    //
    // 7. Save the updated Map to memory
