...
export default async () => {
  ...
  allCalendarDatesRaw.rows.forEach((item) => {
    if (allCalendarDatesRawMap.has(item.service_id)) { allCalendarDatesRawMap.get(item.service_id).push(item.date); }
		 else { allCalendarDatesRawMap.set(item.service_id, [item.date]); }
  });
