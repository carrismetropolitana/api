...
import { SubwayStation, SubwayStationsSource } from '@carrismetropolitana/api-types/src/facilities/facilities.js';
...
export const syncSubwayStations = async () => {
	...
	LOGGER.title(`Sync Subway Stations`);
	...
	const allSourceItems = Papa.parse<SubwayStationsSource>(downloadedSourceText, { header: true });
	...
	const allUpdatedItemsData: SubwayStation[] = [];
	...
	for (const sourceItem of allSourceItems.data) {
		...
		const updatedItemData: SubwayStation = {
			lat: sourceItem.lat,
			location: matchingLocation,
			lon: sourceItem.lon,
			name: sourceItem.name,
			stop_ids: sourceItem.stops?.length ? sourceItem.stops.split('|') : [],
			subway_station_id: sourceItem.id,
		};
	...
	allUpdatedItemsData.sort((a, b) => sortCollator.compare(a.subway_station_id, b.subway_station_id));
	await SERVERDB.set(SERVERDB_KEYS.FACILITIES.SUBWAY_STATIONS, JSON.stringify(allUpdatedItemsData));

	LOGGER.success(`Done updating ${updatedItemsCounter} items to ${SERVERDB_KEYS.FACILITIES.SUBWAY_STATIONS} (${globalTimer.get()}).`);
