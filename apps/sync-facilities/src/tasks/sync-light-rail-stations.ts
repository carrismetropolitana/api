...
import { LightRailStation, LightRailStationsSource } from '@carrismetropolitana/api-types/src/facilities/facilities.js';
...
const DATASET_FILE_URL = 'https://raw.githubusercontent.com/carrismetropolitana/datasets/latest/connections/light_rail_stations/light_rail_stations.csv';

/* * */

export const syncLightRailStations = async () => {
	//

	LOGGER.title(`Sync Light Rail Stations`);
	...
	const allSourceItems = Papa.parse<LightRailStationsSource>(downloadedSourceText, { header: true });
	...
	const allUpdatedItemsData: LightRailStation[] = [];
	...
	for (const sourceItem of allSourceItems.data) {
		...
		const updatedItemData: LightRailStation = {
			lat: sourceItem.lat,
			light_rail_station_id: sourceItem.id,
	...
	allUpdatedItemsData.sort((a, b) => sortCollator.compare(a.light_rail_station_id, b.light_rail_station_id));
	await SERVERDB.set(SERVERDB_KEYS.FACILITIES.LIGHT_RAIL_STATIONS, JSON.stringify(allUpdatedItemsData));

	LOGGER.success(`Done updating ${updatedItemsCounter} items to ${SERVERDB_KEYS.FACILITIES.LIGHT_RAIL_STATIONS} (${globalTimer.get()}).`);
