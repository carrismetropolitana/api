...
export const syncTrainStations = async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.set(`${SERVERDB_KEYS.DATASETS.CONNECTIONS_TRAIN_STATIONS}:${parsedItemData.id}`, JSON.stringify(parsedItemData));
		updatedItemKeys.add(`${SERVERDB_KEYS.DATASETS.CONNECTIONS_TRAIN_STATIONS}:${parsedItemData.id}`);
		//
	}
	...
	await SERVERDB.set(`${SERVERDB_KEYS.DATASETS.CONNECTIONS_TRAIN_STATIONS}:all`, JSON.stringify(allItemsData));
	updatedItemKeys.add(`${SERVERDB_KEYS.DATASETS.CONNECTIONS_TRAIN_STATIONS}:all`);

	// 6.
	// Delete all items not present in the current update

	const allSavedItemKeys = [];
	for await (const key of await SERVERDB.scanIterator({ MATCH: `${SERVERDB_KEYS.DATASETS.CONNECTIONS_TRAIN_STATIONS}:*`, TYPE: 'string' })) {
