...
const index = async (_, reply) => {
	const index = await SERVERDB.get(`timetables:index`);
	return reply
		.code(200)
...
const single = async (request, reply) => {
	// 4512/010136
	const singleItem = await SERVERDB.get(`timetables:${request.params.line_id}/${request.params.direction_id}/${request.params.stop_id}`);
	return reply
		.code(200)
		.header('Content-Type', 'application/json; charset=utf-8')
