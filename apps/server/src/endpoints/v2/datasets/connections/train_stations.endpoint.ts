...
const all = async (_, reply) => {
	const allItems = await SERVERDB.get('v2:datasets:connections:train_stations:all');
	return reply
		.code(200)
...
const single = async (request, reply) => {
	const singleItem = await SERVERDB.get(`v2:datasets:connections:train_stations:${request.params.id}`);
	return reply
		.code(200)
		.header('Content-Type', 'application/json; charset=utf-8')
		.send(singleItem || {});
