...
import { DemandMetricsByAgencyMonth } from '@carrismetropolitana/api-types/metrics';
...
export const syncDemandMetricsByAgencyMonth = async () => {
	//
	...
	const startDateObject = DateTime.fromFormat(currentOperationalDay, 'yyyyLLdd').minus({ month: 1 }).set({ hour: 4, minute: 0, second: 0 });
