...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.client.set(`v2:datasets:connections:boat_stations:${parsedItemData.id}`, JSON.stringify(parsedItemData));
		updatedItemKeys.add(`v2:datasets:connections:boat_stations:${parsedItemData.id}`);
		//
	}
	...
	await SERVERDB.client.set('v2:datasets:connections:boat_stations:all', JSON.stringify(allItemsData));
	updatedItemKeys.add('v2:datasets:connections:boat_stations:all');

	// 6.
	// Delete all items not present in the current update

	const allSavedItemKeys = [];
	for await (const key of SERVERDB.client.scanIterator({ MATCH: 'v2:datasets:connections:boat_stations:*', TYPE: 'string' })) {
