...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.set(`v2:datasets:connections:subway_stations:${parsedItemData.id}`, JSON.stringify(parsedItemData));
	...
	await SERVERDB.set('v2:datasets:connections:subway_stations:all', JSON.stringify(allItemsData));
	...
	for await (const key of await SERVERDB.scanIterator({ MATCH: 'v2:datasets:connections:subway_stations:*', TYPE: 'string' })) {
