...
export const alertsSummary = async () => {
	...
	const filter = {
		active_period_start_date: {
			$gte: startOfYear.unix_milliseconds,
			$lte: yesterdayDate.unix_milliseconds,
		},
	};

	const totalAlerts = await alertsCollection.countDocuments(filter);
	LOGGER.info(`Processing ${totalAlerts} alerts from ${startOfYear.operational_date_int} to ${yesterdayDate.operational_date_int}`);
	...
	const alertsByLineAndDate = new Map<string, HubV1ApiAlert['cause']>();
		...
		if (Array.isArray(alert.references)) {
			for (const ref of alert.references) {
				if (ref.parent_id && alert.active_period_start_date) {
					const date = Dates.fromUnixMilliseconds(alert.active_period_start_date).operational_date_int;
					...
					// Add to daily stats
					if (!dailyStats.has(String(date))) {
						dailyStats.set(String(date), { lines: new Set(), passengers: 0 });
					}
					dailyStats.get(String(date)).lines.add(ref.parent_id);
				}
			}
		}
	}
	...
	for (const date of sortedDailyStats) {
		...
		if (lines.length > 0) {
			const qty = await simplifiedApexValidations.count({
				created_at: {
					$gte: Dates.fromOperationalDateInt(date, 'Europe/Lisbon').startOf('day').unix_milliseconds,
					$lt: Dates.fromOperationalDateInt(date, 'Europe/Lisbon').endOf('day').unix_milliseconds,
				},
				is_passenger: true,
				line_id: { $in: lines },
			});
	...
	// 		tripsByCause: tripsNotMade > 0 && alertsByLineAndDate.has(`${metric.operational_date_int}-${metric.line_id}`)
	// 			? {
	// 				...acc.tripsByCause,
	// 				[alertsByLineAndDate.get(`${metric.operational_date_int}-${metric.line_id}`)]:
	//                     (acc.tripsByCause[alertsByLineAndDate.get(`${metric.operational_date_int}-${metric.line_id}`)] ?? 0) + tripsNotMade,
	// 			}
	// 			: acc.tripsByCause,
	// 	};
