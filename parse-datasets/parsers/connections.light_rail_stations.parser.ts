...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.client.set(`datasets/connections/light_rail_stations/${parsedItemData.id}`, JSON.stringify(parsedItemData));
		updatedItemKeys.add(`datasets/connections/light_rail_stations/${parsedItemData.id}`);
		//
	}
	...
	await SERVERDB.client.set('datasets/connections/light_rail_stations/all', JSON.stringify(allItemsData));
	updatedItemKeys.add('datasets/connections/light_rail_stations/all');

	// 6.
	// Delete all items not present in the current update

	const allSavedItemKeys = [];
	for await (const key of SERVERDB.client.scanIterator({ MATCH: 'datasets/connections/light_rail_stations/*', TYPE: 'string' })) {
