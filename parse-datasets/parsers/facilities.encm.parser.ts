...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.client.set(`datasets/facilities/encm/${parsedItemData.id}`, JSON.stringify(parsedItemData));
		updatedItemKeys.add(`datasets/facilities/encm/${parsedItemData.id}`);
		//
	}
	...
	await SERVERDB.client.set('datasets/facilities/encm/all', JSON.stringify(allItemsData));
	updatedItemKeys.add('datasets/facilities/encm/all');
