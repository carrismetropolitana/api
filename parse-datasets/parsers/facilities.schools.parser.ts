...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.client.set(`datasets/facilities/schools/${parsedItemData.id}`, JSON.stringify(parsedItemData));
		updatedItemKeys.add(`datasets/facilities/schools/${parsedItemData.id}`);
		//
	}
	...
	await SERVERDB.client.set('datasets/facilities/schools/all', JSON.stringify(allItemsData));
	updatedItemKeys.add('datasets/facilities/schools/all');
