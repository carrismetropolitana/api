...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.client.set(`v2:datasets:sla:${parsedItemData.lineId}:${parsedItemData.operationalDay}`, JSON.stringify(parsedItemData));
		...
		updatedItemKeys.add(`v2:datasets:sla:${parsedItemData.lineId}:${parsedItemData.operationalDay}`);
		//
	}
	...
	for (const [lineId, lineData] of lines) {
		await SERVERDB.client.set(`v2:datasets:sla:${lineId}:all`, JSON.stringify(lineData));
	}
	...
	await SERVERDB.client.set('v2:datasets:sla:all', JSON.stringify(allItemsData));
	updatedItemKeys.add('v2:datasets:sla:all');

	// 6.
	// Delete all items not present in the current update

	const allSavedItemKeys = [];
	for await (const key of SERVERDB.client.scanIterator({ MATCH: 'v2:datasets:sla/*', TYPE: 'string' })) {
