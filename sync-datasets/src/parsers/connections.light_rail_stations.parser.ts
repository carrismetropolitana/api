...
export default async () => {
	...
	for (const itemCsv of allItemsCsv.data) {
		...
		await SERVERDB.client.set(`v2:datasets:connections:light_rail_stations:${parsedItemData.id}`, JSON.stringify(parsedItemData));
	...
	await SERVERDB.client.set('v2:datasets:connections:light_rail_stations:all', JSON.stringify(allItemsData));
	...
	for await (const key of SERVERDB.client.scanIterator({ MATCH: 'v2:datasets:connections:light_rail_stations:*', TYPE: 'string' })) {
