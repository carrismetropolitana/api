				...
				const subConditions = Object.entries(value).map(([operator, val]) => {
					switch (operator) {
						...
						case '$gte':
							return `${key} >= ${this.formatValue(val)}`;
						case '$in':
							return `${key} IN (${Array.isArray(val) ? val.map(v => this.formatValue(v)).join(', ') : this.formatValue(val)})`;
						case '$lt':
							return `${key} < ${this.formatValue(val)}`;
						case '$lte':
							return `${key} <= ${this.formatValue(val)}`;
						case '$ne':
							return `${key} != ${this.formatValue(val)}`;
						case '$nin':
							return `${key} NOT IN (${Array.isArray(val) ? val.map(v => this.formatValue(v)).join(', ') : this.formatValue(val)})`;
						default:
